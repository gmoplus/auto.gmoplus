    ...
    public function loadListings($limit = 1, $options = false, $number = false, $priceTag = false, $side_bar_exists = false, $page_info_key = false)
    {
        ...
        if ($options[0] == 'featured') {
            ...
            if ($options[4]) {
                ...
                $rlListings->selectedIDs = $_SESSION['carousel'][$options[1]] ? : $_SESSION['carousel']['all_ids'];
            }
        ...
        if ($listings) {
            ...
            $rlSmarty->assign('block', $GLOBALS['rlDb']->fetch("*", array('Status' => 'active', 'Key' => $options[1]), null, 1, 'blocks', 'row'));
