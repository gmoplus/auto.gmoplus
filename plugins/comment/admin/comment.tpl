...
<div id="nav_bar">
    {if !$smarty.get.action}
        <a href="javascript:void(0)" onclick="show('search')" class="button_bar">
    ...
    {else}
        <a href="{$rlBase}index.php?controller={$smarty.get.controller}" class="button_bar">
            <span class="left"></span><span class="center_list">{$lang.items_list}</span><span class="right"></span>
        </a>
    {/if}
</div>
<!-- navigation bar end -->
        ...
        <table class="form">
            ...
            <tr>
                ...
                <td class="field">
                    ...
                    <textarea name="description" rows="10" cols=""
                        style="height: 150px;">{$smarty.post.description}</textarea>
                </td>
            </tr>
            <tr>
                ...
                <td class="field">
                    <select name="status">
                        <option value="active" {if $sPost.status == 'active'}selected="selected" {/if}>{$lang.active}
                        </option>
                        <option value="approval" {if $sPost.status == 'approval'}selected="selected" {/if}>{$lang.approval}
                        </option>
                    </select>
                </td>
            </tr>
    ...
    <div id="search" class="hide">
        ...
        <table class="form">
                ...
                <td class="field">
                    <select id="listing_type" style="width: 200px;">
                        ...
                        {foreach from=$listing_types item='listing_type'}
                            <option value="{$listing_type.Key}"
                                {if $listing_type.Key == $smarty.get.listing_type}selected="selected" {/if}>{$listing_type.name}
                            </option>
                        {/foreach}
                    </select>
                </td>
            ...
            <tr>
                ...
                <td class="field">
                    <input style="width: 65px;" type="text" value="{$smarty.post.date_from}" size="12" maxlength="10"
                        id="date_from" />
                    <img class="divider" alt="" src="{$rlTplBase}img/blank.gif" />
                    <input style="width: 65px;" type="text" value="{$smarty.post.date_to}" size="12" maxlength="10"
                        id="date_to" />
                </td>
            </tr>
            <tr>
                ...
                <td class="field">
                    <select id="search_status" style="width: 200px;">
                        ...
                        {foreach from=$statuses item='user_status'}
                            <option value="{$user_status}" {if $user_status == $smarty.get.status}selected="selected" {/if}>
                                {$lang.$user_status}</option>
                        {/foreach}
                    </select>
                </td>
            </tr>
            ...
            $(document).ready(function() {
                commentsGrid = new gridObj({
                    ...
                    columns: [{
                        ...
                        header: '{/literal}{$lang.comment_rating}{literal}',
                        dataIndex: 'Rating',
                        width: 80,
                        fixed: true
                    }, {
                        ...
                        renderer: function(id) {
                            ...
                            out += "<a href='" + rlUrlHome + "index.php?controller=" + controller +
                                "&action=edit&id=" + id + "'><img class='edit' ext:qtip='" + lang[
                                    'ext_edit'] + "' src='" + rlUrlHome + "img/blank.gif' /></a>";
                            out += "<img class='remove' ext:qtip='" + lang['ext_delete'] +
                                "' src='" + rlUrlHome + "img/blank.gif' onClick='rlConfirm( \"" +
                                lang['ext_notice_delete'] + "\", \"deleteComment\", \"" + id + "\" )' />";
                            out += "</center>";
