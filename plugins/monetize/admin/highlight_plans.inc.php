...
if ($_GET['q'] == 'ext') {
    ...
    if ($_GET['action'] == 'update') {
        ...
        $sql = "SELECT COUNT(`ID`) AS `count` FROM `{db_prefix}monetize_plans` WHERE `Type` = 'highlight'";
        $count = $rlDb->getRow($sql);

        $output['total'] = $count['count'];
        $data = $rlMonetize->getPlans($start, $limit, null, 'highlight');
    ...
    if ($_GET['action'] == 'edit' && !$_POST['edit']) {
        ...
        $_POST['highlight_count'] = $plan_info['Highlights'];
        $_POST['highlight_count_unlimited'] = !(bool) $plan_info['Highlights'];
        $_POST['price'] = $plan_info['Price'];
        $_POST['color'] = $plan_info['Color'];
        $_POST['highlight_days'] = $plan_info['Days'];
        $_POST['status'] = $plan_info['Status'];

        // get name
        $where = array('Key' => 'highlight_plan+name+' . $plan_info['Key']);
        ...
        // get description
        $where = array('Key' => 'highlight_plan+description+' . $plan_info['Key']);
