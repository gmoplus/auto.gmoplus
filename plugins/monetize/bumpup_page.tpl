{if $smarty.get.id && !isset($smarty.get.completed)}
     {if $plans}
        ...
        <form id="form-checkout" method="post" action="{$formAction}">
            ...
            {if $plans|@count > 1}{assign var='fieldset_phrase' value=$lang.select_bump_up}{else}{assign var='fieldset_phrase' value=$plans.$firstIndex.name}{/if}
            ...
            <div class="plans-container">
                ...
                {else}
                    ...
                    <div class="table-cell clearfix">
                        ...
                        <div class="value">
                            {if $plans.$firstIndex.Using_ID && ($plans.$firstIndex.Bumpups_available > 0 || $plans.$firstIndex.Is_unlim == 1)}
                                &#8212;
                    ...
                    <div class="table-cell clearfix">
                        <div class="name"><div><span>{$lang.bumpups_available}</span></div></div>
                        <div class="value">
                            {if $plans.$firstIndex.Bump_ups > 0}
                                {if $plans.$firstIndex.Bumpups_available}
                                    {assign var='bAvailable' value=$plans.$firstIndex.Bumpups_available}
                                {else}
                                    {assign var='bAvailable' value=$plans.$firstIndex.Bump_ups}
                                {/if}
                                {assign var='rRest' value=`$smarty.ldelim`rest`$smarty.rdelim`}
                                {assign var='rAmount' value=`$smarty.ldelim`amount`$smarty.rdelim`}
                                {$lang.rest_of_amount|replace:$rRest:$bAvailable|replace:$rAmount:$plans.$firstIndex.Bump_ups}
                            {else}
                                {$lang.unlimited}
                            {/if}
                        </div>
