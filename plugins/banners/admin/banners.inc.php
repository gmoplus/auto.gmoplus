...
if ($_GET['q'] == 'ext') {
    ...
    if ($_GET['action'] == 'update') {
        ...
        $sql = "SELECT `T1`.`Account_ID`, `T1`.`Date_to`, `T2`.`Plan_Type`, `T2`.`Period`, `T2`.`Price`, `T1`.`Status` FROM `{db_prefix}banners` AS `T1` ";
        ...
        if ($field == 'Pay_date' || ($field == 'Status' && $value == 'active')) {
            $date_to = ($bannerInfo['Period'] != 0
                ? ($bannerInfo['Plan_Type'] == 'period'
                    ? time() + ($bannerInfo['Period'] * 86400)
        ...
        $action = $rlDb->updateOne($updateData, 'banners');
        ...
        if ($action && $field == 'Status') {
            ...
            $mail_tpl['body'] = str_replace('{name}', $accountInfo['Full_name'], $mail_tpl['body']);
    ...
    $sql .= "`T1`.`Pay_date`, `T2`.`Value` AS `name`, `T3`.`Username`, `T4`.`Key` AS `Plan_key`, `T4`.`Price`, `T4`.`Period`, `T4`.`Plan_Type` ";
    $sql .= "FROM `{db_prefix}banners` AS `T1` ";
    $sql .= "LEFT JOIN `{db_prefix}lang_keys` AS `T2` ON CONCAT('banners+name+',`T1`.`ID`) = `T2`.`Key` AND `T2`.`Code` = '" . RL_LANG_CODE . "' ";
    $sql .= "LEFT JOIN `{db_prefix}accounts` AS `T3` ON `T1`.`Account_ID` = `T3`.`ID` ";
    $sql .= "LEFT JOIN `{db_prefix}banner_plans` AS `T4` ON `T1`.`Plan_ID` = `T4`.`ID` ";
    $sql .= "WHERE `T1`.`Status` <> 'trash' AND `Account_ID` <> 0 ";
    ...
    foreach ($data as $key => $value) {
        // get banner clicks
        $sql = "SELECT COUNT(`ID`) AS `total` FROM `{db_prefix}banners_click` ";
        ...
        if ($data[$key]['Type'] == 'html') {
            $data[$key]['thumbnail'] = $lang['banners_html_code_blank_flange'];
        } else {
            $src = RL_FILES_URL . 'banners/' . $data[$key]['thumbnail'];

            if (!empty($data[$key]['thumbnail']) && file_exists(RL_FILES . 'banners/' . $data[$key]['thumbnail'])) {
                ...
                $data[$key]['thumbnail'] = $lang['banners_file_missing'];
            }
        }
...
if (isset($_GET['module'])) {
    if (file_exists($module = RL_PLUGINS . 'banners/admin/' . $_GET['module'] . '.inc.php')) {
        include $module;
    } else {
        $sError = true;
    }
        ...
        if (empty($plans)) {
            ...
            foreach ($plans as $key => $entry) {
                ...
                foreach ($boxes as $box) {
                    $boxInfo = $rlDb->getRow("SELECT `Side`, `Banners` FROM `{db_prefix}blocks` WHERE `Key` = '{$box}' AND `Plugin` = 'banners'");
            ...
            $rlSmarty->assign('plansJson', json_encode($plansJson));
            unset($plansJson);
            ...
            if ($_GET['action'] == 'edit' && !$_POST['postSubmit']) {
                ...
                $sql .= "`T1`.`Html`, `T1`.`Responsive`, `T1`.`Status`, `T2`.`Username`, `T1`.`Image` ";
                $sql .= "FROM `{db_prefix}banners` AS `T1` ";
                $sql .= "LEFT JOIN `{db_prefix}accounts` AS `T2` ON `T1`.`Account_ID` = `T2`.`ID` AND `T2`.`Status` = 'active' ";
                ...
                if ($savedBannerInfo['Type'] == 'html') {
                    ...
                    $_POST['responsive'] = (int) $savedBannerInfo['Responsive'];
                } else {
                    $_POST['image'] = $savedBannerInfo['Image'];
                }

                if (count($allLangs) > 1) {
                    $names = $rlDb->fetch(['Value', 'Code'], ['Key' => "banners+name+{$bannerId}"], "AND `Status` <> 'trash'", null, 'lang_keys');
            ...
            if ($_POST['submit']) {
                ...
                if (!empty($errors)) {
                    ...
                    $reefless->redirect(['controller' => $controller]);
                }
            }
        }
    } else {
