...
class rlBanners extends Flynax\Abstracts\AbstractPlugin implements Flynax\Interfaces\PluginInterface
{
    ...
    public function bannersInBox($boxKey, $limit = 1, $externalGeoData = false)
    {
        ...
        if ($externalGeoData) {
            $geo_location = $externalGeoData;
        } elseif ($rlGeoFilter && $rlGeoFilter->geo_filter_data['location']) {
            $geo_location = $rlGeoFilter->geo_filter_data['location'];
        }
        $countryCode = $_SESSION['GEOLocationData']->Country_code;
        $geo_filter = 1;

        if ($geo_location && $this->mfActive()) {
    ...
    public function getBanners($boxKey, $limit = 1)
    {
        if (!empty($this->bannersList[$boxKey])) {
            $banners = array_slice($this->bannersList[$boxKey], 0, $limit);
...
JS;
        ...
        if ($rlDb->query($sql)) {
            ...
            if ($langKeysInsert) {
                if (method_exists($rlLang, 'createPhrases')) {
                    $rlLang->createPhrases($langKeysInsert);
                } else {
    ...
    public function bannerNameHandler($bannerId = false, $names = false)
    {
        ...
        if ($langKeysUpdate) {
            if (method_exists($rlLang, 'updatePhrases')) {
                $rlLang->updatePhrases($langKeysUpdate);
            } else {
    ...
    public function deleteBanner($id, $date)
    {
        ...
        if (method_exists($rlLang, 'deletePhrase')) {
            $rlLang->deletePhrase(['Key' => "banners+name+{$id}", 'Plugin' => 'banners']);
        } else {
    ...
    public function mfParentPoints($locations)
    {
        ...
        $sql = "SELECT `Parent_IDs` FROM `{db_prefix}multi_formats` ";
        ...
        foreach ($locations as $location) {
            ...
            $ids = $rlDb->getAll("
                SELECT `Key` FROM `{db_prefix}multi_formats`
