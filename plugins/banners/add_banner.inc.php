...
if (!$currentStep) {
    Util::redirect($reefless->getPageUrl($page_info['Key'], ['step' => $addBannerSteps['plan']['path']]));
}
...
$returnLink = $reefless->getPageUrl($page_info['Key'], ['step' => $addBannerSteps['plan']['path']]);
$rlSmarty->assign('returnLink', $returnLink);

if (!$planId && $currentStep != 'plan') {
    Util::redirect($returnLink);
}
...
switch ($currentStep) {
    case 'plan':
        ...
        if ($_POST['step'] == 'plan') {
            ...
            if (empty($errors)) {
                ...
                Util::redirect($reefless->getPageUrl($page_info['Key'], ['step' => $addBannerSteps['form']['path']]));
            }
        }
        break;
    ...
    case 'form':
        ...
        if ($_POST['step'] == 'form') {
            ...
            if (empty($errors)) {
                ...
                Util::redirect($reefless->getPageUrl($page_info['Key'], ['step' => $nextStep['path']]));
            }
        }
        break;
    ...
    case 'checkout':
        ...
        $success_url = $reefless->getPageUrl($page_info['Key'], ['step' => $nextStep['path']]);
        $cancel_url  = $reefless->getPageUrl($page_info['Key'], ['step' => $currentStep], null, 'canceled');
