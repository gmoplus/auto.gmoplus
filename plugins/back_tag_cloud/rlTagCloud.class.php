...
class rlTagCloud
{
    ...
    public function ajaxDeleteTag($key = false)
    {
        ...
        $GLOBALS['rlActions']->delete(['Key' => $key], ['tag_cloud'], null, 1, $key, $lang_keys);
    ...
    {
        foreach ($GLOBALS['data'] as &$trashItem) {
            if ($trashItem['Zones'] === 'tag_cloud' && !$trashItem['Item']) {
                $trashItem['Item'] = $GLOBALS['rlDb']->getOne('Tag', $trashItem['Criterion'], 'tag_cloud');
            }
        }
    }
